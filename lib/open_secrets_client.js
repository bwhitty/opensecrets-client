...
var OpenSecretsClient = rest.service(function(apiKey, format) {
   ...
   * @param {{}} options
   */
  getCandContrib: function (options) {
    return this.get(this._makeUrl(extend(options, {method: 'candContrib'})));
  },

  /**
   * GET legislators for a given state, district, or CID.
   *
   * @param {{}} options
   * @returns {*}
   */
  getLegislators: function (options) {
    return this.get(this._makeUrl(extend(options, {method: 'getLegislators'})));
  },
